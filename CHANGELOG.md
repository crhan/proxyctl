...
- **新增 `proxy_env_extra` 配置：`proxyctl env` 可以额外导出指定的代理变量。**
  列出的变量名会被 `proxyctl env` 导出为引擎的 HTTP 代理地址
  （`http://127.0.0.1:<proxy_port>`），`proxyctl env --unset` 一并清除。
...
- **`proxyctl check` 连通性面板的 `via` 列不再显示热 reload 前遗留的旧路由。**
  mihomo 热 reload 不会掐断旧 keep-alive 连接，规则改过之后旧连接仍挂着旧
  路由链；反查 `/connections` 时命中第一条就返回，会把遗留连接的旧链路当成
  当前路由显示。现在在所有 host 匹配的连接里取 `start` 最新的一条，`via`
  显示的始终是当前规则下的真实路由。
...
- `get_engine_version()` 兼容两种 mihomo `-v` 输出格式：

  | 格式 | 样本 |
  |---|---|
  | ISO 风 | `Mihomo Meta 1.19.25 darwin arm64 with go1.26.3 2026-05-16T14:37:07Z` |
  | v 前缀 + Unix `date` 风 | `Mihomo Meta v1.19.10 linux amd64 with go1.24.3 Sat May 31 07:51:30 UTC 2025` |

  v 前缀 + Unix date 构建之前会解析成 `version="v1.19.10"` /
  `build_date="Sat"`，导致 `status` / `doctor` 显示
  `engine=mihomo vv1.19.10 (Sat, go1.24.3)`（双 `v` + build_date 截断）。

- `data.engine_version.version` 始终不带 `v` 前缀（解析时 `lstrip("v")`
  规范化），渲染层统一拼 `v{version}`。
- `build_date` 解析吞到行尾，容纳 Unix `date` 的多 token 格式。
- ISO 风 build_date 判定收紧为 `^\d{4}-\d{2}-\d{2}T` 头，避免 `UTC`
  字符串中的 `T` 把 build_date 截断成 `'Sat May 31 07:51:30 U'`。

### Added

- `tests/unit/test_cli_helpers.py` parametrize 两种 mihomo `-v` 输出格式，
  含 `not info["version"].startswith("v")` 反向断言防止值再含 v 前缀。

### Compatibility

- Envelope `schema_version` 不变；`data.engine_version` 字段集与类型不变
  （仅 `version` 字段值规范化）。Agent 消费方无感升级。
